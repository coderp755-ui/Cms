...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--name", type=str, help='Branch name (e.g., "Kathmandu Branch")'
        )
    ...
    def handle(self, *args, **options):
        ...
        if not name or not code:
            ...
            self.stdout.write(
                '  python manage.py create_branch --name "Branch Name" --code "CODE"'
            )
            self.stdout.write("\nOptional arguments:")
        ...
        if Branch.objects.filter(code=code).exists():
            self.stdout.write(
                self.style.ERROR(f'❌ Branch with code "{code}" already exists!')
            )
            return
