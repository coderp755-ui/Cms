...
from apps.tests.models import (
    ...
    StudentAnswer, TestResult, BandScoreMapping
)
...
class Command(BaseCommand):
    ...
    def create_branches(self):
        ...
        branch_data = [
            {'name': 'Kathmandu Branch', 'code': 'KTM', 'address': 'Thamel, Kathmandu', 'phone': '+977-1-4123456', 'email': 'ktm@grace.edu.np'},
            {'name': 'Pokhara Branch', 'code': 'PKR', 'address': 'Lakeside, Pokhara', 'phone': '+977-61-123456', 'email': 'pkr@grace.edu.np'},
            {'name': 'Lalitpur Branch', 'code': 'LTP', 'address': 'Jawalakhel, Lalitpur', 'phone': '+977-1-5123456', 'email': 'ltp@grace.edu.np'},
        ]
    ...
    def create_users(self):
        ...
        for idx, branch in enumerate(self.branches, 1):
            ...
            if not User.objects.filter(username=username).exists():
                ...
                self.stdout.write(f'  ✓ Created admin: {admin.username} for {branch.name}')
        
        # Create teachers for each branch
        teacher_data = [
            {'first_name': 'John', 'last_name': 'Smith', 'dept': 'IELTS', 'subject': 'Speaking & Writing'},
            {'first_name': 'Sarah', 'last_name': 'Johnson', 'dept': 'PTE', 'subject': 'Reading & Listening'},
            {'first_name': 'Michael', 'last_name': 'Brown', 'dept': 'IELTS', 'subject': 'All Sections'},
        ]
        ...
        for branch in self.branches:
            for data in teacher_data[:2]:  # 2 teachers per branch
                ...
                if not User.objects.filter(username=username).exists():
                    ...
                    self.stdout.write(f'  ✓ Created teacher: {teacher.username} for {branch.name}')
                    teacher_count += 1
        ...
        for branch in self.branches:
            for data in student_data[:3]:  # 3 students per branch
                ...
                if not User.objects.filter(username=username).exists():
                    ...
                    self.stdout.write(f'  ✓ Created student: {student.username} for {branch.name}')
                    student_count += 1
        ...
        for branch in self.branches:
            for data in course_data:
                ...
                if created:
                    ...
                    self.stdout.write(f'    ✓ Enrolled {students.count()} students in {course.title}')
            ...
            if created:
                ...
                for i in range(1, 6):  # 5 questions per section
                    ...
                    question = Question.objects.create(
                        ...
                        marks=2,
                        order=i,
                        correct_answer='Sample correct answer' if q_type == 'text' else None
                    )
    ...
    def create_test_attempts(self):
        ...
        for student in students[:2]:  # First 2 students
            for test in tests:
                ...
                if created:
                    self.stdout.write(f'  ✓ Created attempt: {student.username} - {test.title}')
                    ...
                    for question in questions:
                        ...
                        if question.question_type == 'mcq':
                            ...
                            answer_data['selected_option'] = correct_option if is_correct else question.options.exclude(is_correct=True).first()
                        else:
                    ...
                    self.stdout.write(f'    ✓ Created result: {obtained_marks}/{total_marks} ({percentage:.1f}%)')
    ...
    def create_band_mappings(self):
        ...
        mappings = [
            # IELTS Listening
            {'test_type': 'IELTS', 'section': 'listening', 'min_score': 39, 'max_score': 40, 'band_score': Decimal('9.0')},
            {'test_type': 'IELTS', 'section': 'listening', 'min_score': 35, 'max_score': 38, 'band_score': Decimal('8.0')},
            {'test_type': 'IELTS', 'section': 'listening', 'min_score': 30, 'max_score': 34, 'band_score': Decimal('7.0')},
            {'test_type': 'IELTS', 'section': 'listening', 'min_score': 23, 'max_score': 29, 'band_score': Decimal('6.0')},
            
            # IELTS Reading
            {'test_type': 'IELTS', 'section': 'reading', 'min_score': 39, 'max_score': 40, 'band_score': Decimal('9.0')},
            {'test_type': 'IELTS', 'section': 'reading', 'min_score': 35, 'max_score': 38, 'band_score': Decimal('8.0')},
            {'test_type': 'IELTS', 'section': 'reading', 'min_score': 30, 'max_score': 34, 'band_score': Decimal('7.0')},
            {'test_type': 'IELTS', 'section': 'reading', 'min_score': 23, 'max_score': 29, 'band_score': Decimal('6.0')},
        ]
        
        for mapping in mappings:
            obj, created = BandScoreMapping.objects.get_or_create(**mapping)
            if created:
                self.stdout.write(f'  ✓ Created mapping: {mapping["section"]} {mapping["min_score"]}-{mapping["max_score"]} → {mapping["band_score"]}')
    ...
    def create_lesson_progress(self):
        ...
        for student in students[:2]:  # First 2 students
            for lesson in lessons[:5]:  # Complete first 5 lessons
                ...
                if created:
                    self.stdout.write(f'  ✓ Progress: {student.username} completed {lesson.title}')
