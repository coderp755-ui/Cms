...
class Command(BaseCommand):
        ...
        for branch in self.branches:
            for data in course_data:
                ...
                course, created = Course.objects.get_or_create(
                    ...
                    branch=branch,
                    defaults={
                        "description": f"Complete {data['type']} preparation course"
                    },
                )
            ...
            for test in selected_tests:
                ...
                for question in questions:
                    ...
                    if question.question_type == "mcq":
                        if is_correct:
                            answer_data["selected_option"] = question.options.filter(
                                is_correct=True
                            ).first()
                        else:
                            answer_data["selected_option"] = question.options.filter(
    ...
    def print_summary(self):
        ...
        self.stdout.write(
            f"  - Superadmins: {User.objects.filter(role='superadmin').count()}"
        )
